...
The color of the progress bar can be set with the B<highlight> color. The frame
color can be set with B<frame_color>.

=item B<progress_bar_alignment> (values: [left/right/center], default: center)

Set the alignment of the progress bar.
...
Defines color of the frame around the notification window and the progress bar.
See COLORS.
